...
//        var_dump($authorSubmission->getSHA256());
    ...
    function updateRevisions($args, $request)
    {
        $articleId = (int)array_shift($args);
        $usr = array_shift($args);
        $pwd = array_shift($args);
        $stor = new StorageUiLibrary();
        $revs = $stor->updateRevisions($articleId, $usr, $pwd);
        if ($revs != false){print $revs;
        return;}
        $coms = $stor->updateComments($articleId, $usr, $pwd);
        if ($coms != false){ print $coms;
        return;}
        print json_encode(array(errors => false));
    }

    function updateComments($args, $request)
    {
        $articleId = (int)array_shift($args);
        $usr = array_shift($args);
        $pwd = array_shift($args);
//        $stor = new StorageUiLibrary();
//        $stor->updateComments($articleId, $usr, $pwd);
