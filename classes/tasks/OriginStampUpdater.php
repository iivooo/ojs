...
class OriginStampUpdater extends ScheduledTask {
	 ...
	 */
	function getName() {
		return __('admin.scheduledTask.originStampUpdater');
	}
	
	function hashRequest($hash) {
		
		# Create a connection
		$url = "https://api.originstamp.org/api/";
		$apiKey = "988e7238-995e-4db0-8277-ce8f75d4b037";
		$opts = array('http' =>
				array(
						'method'  => 'POST',
						'header'  => 'Content-type: application/json; charset=UTF-8, Authorization: '.$apiKey ,
						'content' => $postString
				)
		);
		# Create the context
		...
		$result = file_get_contents($url.$hash, false, $context);
	}
